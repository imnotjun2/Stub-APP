...
- The expanded demo should include stable movie-format tags, distinct flight/train details, multiple memory-photo attachments, a poster candidate/confirmation experience, a Flighty-inspired flight card without pretending to provide live flight status, map-first trip books, a visual wall, improved semantic dark mode, and a Chinese/English UI switch. User-authored titles and notes are never auto-translated.
- Offline movie-poster suggestions must be title-matched and explainable. If the bundled catalog has no reliable match, show no poster candidate and invite manual upload; never reuse an unrelated sample poster. The demo may show the verified `MU`, `CA`, and `CZ` airline identification marks with attribution, while every unsupported IATA code falls back to a text badge. A production release still requires a licensed brand-data provider.
